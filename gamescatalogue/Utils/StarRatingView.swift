...
class StarRatingView: UIView {
    ...
    @IBInspectable var starColor: UIColor = UIColor.systemOrange {
        didSet {
            for starImageView in [hstack?.oneStar, hstack?.twoStar, hstack?.threeStar, hstack?.fourStar, hstack?.fiveStar] {
    ...
    fileprivate func setupView(rating: Float, color: UIColor, starRounding: StarRounding) {
        ...
        let nibRating = UINib(nibName: "RatingStackView", bundle: bundle)
        if let viewFromNib = nibRating.instantiate(withOwner: self, options: nil)[0] as? StarRatingStackView {
    ...
    fileprivate func setStarsFor(rating: Float) {
        let starImageViews = [hstack?.oneStar, hstack?.twoStar, hstack?.threeStar, hstack?.fourStar, hstack?.fiveStar]
        for index in 1...5 {
    ...
    fileprivate func touched(touch: UITouch, moveTouch: Bool) {
        ...
        guard let stackView = self.hstack else { return }
        let touchX = touch.location(in: stackView).x
        let ratingFromTouch = 5*touchX/stackView.frame.width
